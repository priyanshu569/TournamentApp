// Generates the legal pages (privacy/terms/delete-account) that live inside
// the marketing-site/ repo, from the SAME section arrays the in-app screens
// render. Legal text that exists twice will eventually say two different
// things, and for a privacy policy that's a real problem -- Play compares
// the hosted policy against your Data Safety answers. So this parses
// app/privacy-policy.tsx and app/terms-of-service.tsx rather than
// duplicating their content here.
//
// These pages share one domain/repo with the marketing landing page
// (marketing-site/index.html) -- same nav, same footer, same brand chrome --
// rather than living on a separate site, so there's one URL to remember and
// one GitHub Pages setup instead of two.
//
...
const LOGO_SYMBOL = `<svg width="0" height="0" style="position:absolute" aria-hidden="true" focusable="false">
    <defs>
      <symbol id="fragifyMark" viewBox="241.68 55.54 771.87 771.87">
        ...
        <defs>
        <linearGradient id="markPaint0" x1="649.3" y1="717.725" x2="648.173" y2="276.645" gradientUnits="userSpaceOnUse"><stop stop-color="#D6D5D6"/><stop offset="1" stop-color="white"/></linearGradient>
        <linearGradient id="markPaint1" x1="784.038" y1="406.399" x2="873.092" y2="232.757" gradientUnits="userSpaceOnUse"><stop stop-color="#6A19D0"/><stop offset="1" stop-color="#A843FF"/></linearGradient>
        <linearGradient id="markPaint2" x1="411.36" y1="429.17" x2="298.53" y2="305.113" gradientUnits="userSpaceOnUse"><stop stop-color="#6714D5"/><stop offset="1" stop-color="#A944FF"/></linearGradient>
        <linearGradient id="markPaint3" x1="921.935" y1="593.403" x2="896.481" y2="461.822" gradientUnits="userSpaceOnUse"><stop stop-color="#2E0369"/><stop offset="1" stop-color="#7220D3"/></linearGradient>
        <linearGradient id="markPaint4" x1="589.297" y1="190.606" x2="596.061" y2="287.66" gradientUnits="userSpaceOnUse"><stop stop-color="#111121"/><stop offset="1" stop-color="#2C2B42"/></linearGradient>
        <linearGradient id="markPaint5" x1="822.442" y1="736.419" x2="807.214" y2="593.917" gradientUnits="userSpaceOnUse"><stop stop-color="#010019"/><stop offset="1" stop-color="#2C056A"/></linearGradient>
        <linearGradient id="markPaint6" x1="808.965" y1="420.509" x2="884.076" y2="432.979" gradientUnits="userSpaceOnUse"><stop stop-color="#0F0F1D"/><stop offset="1" stop-color="#27273F"/></linearGradient>
        <clipPath id="markClip"><rect width="1254" height="1254" fill="white" transform="translate(241.68 55.54)"/></clipPath>
        </defs>
      </symbol>
    </defs>
  </svg>`;
const LOGO_USE = `<svg><use href="#fragifyMark"></use></svg>`;
...
// Same design system as marketing-site/index.html (tokens, fonts, nav,
// footer) so these pages read as part of one site rather than a bolted-on
// legal doc. Content styling underneath is deliberately calmer than the
...
const SITE_STYLE = `
  ...
  * { box-sizing: border-box; }
  ...
  /* Same ambient glow as the landing page, so this reads as one site. */
  body::before, body::after {
    content: ''; position: fixed; z-index: 0; border-radius: 50%;
    filter: blur(90px); pointer-events: none;
  }
  body::before { width: 520px; height: 520px; top: -220px; left: -160px; background: radial-gradient(circle, rgba(139,92,246,0.30), transparent 70%); }
  body::after { width: 460px; height: 460px; bottom: -200px; right: -160px; background: radial-gradient(circle, rgba(192,132,252,0.18), transparent 70%); }
  ...
  .nav-inner { max-width: 1120px; margin: 0 auto; padding: 14px 24px; display: flex; align-items: center; gap: 20px; }
  ...
  .brand .wordmark { font: 800 15px/1 var(--font-display); letter-spacing: 2px; color: var(--ink); }
  .nav-links { display: flex; align-items: center; gap: 26px; margin: 0 auto 0 32px; flex: 1; }
  .nav-links a { text-decoration: none; color: var(--ink-dim); font-size: 13.5px; font-weight: 600; letter-spacing: .2px; transition: color .15s ease; }
  .nav-links a:hover { color: var(--violet); }
  .cta-btn {
    text-decoration: none; font-size: 13px; font-weight: 700; letter-spacing: .2px;
  ...
  .cta-btn:hover { transform: translateY(-1px); box-shadow: 0 10px 24px color-mix(in srgb, var(--violet) 45%, transparent); }
  .nav-toggle { display: none; }
  @media (max-width: 780px) {
    .nav-links { display: none; }
    .nav-toggle { display: grid; place-items: center; width: 36px; height: 36px; border-radius: 9px; background: var(--panel-alt); border: 1px solid var(--line); margin-left: auto; cursor: pointer; }
    .nav-toggle span, .nav-toggle span::before, .nav-toggle span::after { content: ''; display: block; width: 16px; height: 2px; background: var(--ink); position: relative; }
    .nav-toggle span::before { position: absolute; top: -5px; }
    .nav-toggle span::after { position: absolute; top: 5px; }
    .nav.open .nav-links { display: flex; flex-direction: column; align-items: flex-start; gap: 4px; position: absolute; top: 100%; left: 0; right: 0; background: var(--ground); border-bottom: 1px solid var(--line); padding: 10px 24px 18px; margin: 0; }
  ...
  h1 {
    font: 800 clamp(1.75rem, 2.6vw + 1rem, 2.5rem) / 1.15 var(--font-display); letter-spacing: -0.5px; margin: 28px 0 10px;
    background: linear-gradient(90deg, var(--ink), var(--violet-2) 130%);
    -webkit-background-clip: text; background-clip: text; color: transparent;
    text-wrap: balance;
  }
  .updated {
    display: inline-flex; align-items: center; gap: 6px;
    color: var(--violet-2); font: 700 12px/1 var(--font-mono); letter-spacing: .3px;
    background: var(--violet-soft); border: 1px solid color-mix(in srgb, var(--violet) 30%, transparent);
    padding: 5px 11px; border-radius: 20px; margin: 0 0 22px;
  }
  .intro { color: var(--ink-dim); margin: 0 0 40px; font-size: 16.5px; line-height: 1.65; max-width: 54ch; }

  /* ---------- Document cards ---------- */
  .doc-card {
    position: relative; background: linear-gradient(180deg, var(--panel), color-mix(in srgb, var(--panel) 92%, var(--panel-alt)));
    border: 1px solid var(--line); border-radius: 16px;
    padding: 24px 26px; margin-bottom: 16px;
    transition: border-color .18s ease, transform .18s ease, box-shadow .18s ease;
  }
  .doc-card:hover { border-color: color-mix(in srgb, var(--violet) 40%, var(--line)); transform: translateY(-2px); box-shadow: 0 14px 34px color-mix(in srgb, var(--violet) 12%, transparent); }
  .doc-card::before, .doc-card::after {
    content: ''; position: absolute; width: 16px; height: 16px; pointer-events: none;
    border-color: color-mix(in srgb, var(--violet) 50%, transparent); transition: border-color .18s ease;
  }
  .doc-card::before { top: -1px; left: -1px; border-top: 2px solid; border-left: 2px solid; border-radius: 7px 0 0 0; }
  .doc-card::after { bottom: -1px; right: -1px; border-bottom: 2px solid; border-right: 2px solid; border-radius: 0 0 7px 0; }
  .doc-card:hover::before, .doc-card:hover::after { border-color: var(--violet); }
  .doc-card h2 { display: flex; align-items: center; gap: 12px; color: var(--ink); font-size: 16px; font-weight: 700; letter-spacing: .2px; margin: 0 0 14px; }
  .doc-card h2 .ico { width: 30px; height: 30px; border-radius: 9px; flex: none; background: var(--violet-soft); border: 1px solid color-mix(in srgb, var(--violet) 30%, transparent); display: grid; place-items: center; font-size: 14px; line-height: 1; }
  .doc-card p { color: var(--ink-dim); margin: 0 0 12px; font-size: 14.5px; line-height: 1.6; }
  .doc-card p:last-child { margin-bottom: 0; }
  a.inline { color: var(--violet-2); text-decoration: none; }
  a.inline:hover { text-decoration: underline; }

  /* ---------- Reveal on scroll (same as the landing page) ---------- */
  ...
  .footer-grid { max-width: 1120px; margin: 0 auto 40px; padding: 0 24px; display: grid; grid-template-columns: 1.4fr 1fr 1fr; gap: 40px; }
  @media (max-width: 700px) { .footer-grid { grid-template-columns: 1fr; gap: 30px; } }
  .footer-brand { display: flex; align-items: center; gap: 10px; margin-bottom: 12px; }
  .footer-brand .mark { width: 34px; height: 34px; border-radius: 10px; background: linear-gradient(150deg, #1c1030, #0a0614); display: grid; place-items: center; padding: 6px; box-shadow: 0 0 0 1px rgba(139,92,246,0.35), 0 6px 18px rgba(139,92,246,0.2); flex: none; }
  .footer-brand .mark svg { width: 100%; height: 100%; display: block; }
  .footer-brand .wordmark { font: 800 14px/1 var(--font-display); letter-spacing: 1.6px; }
  .footer-tag { color: var(--ink-dim); font-size: 13.5px; max-width: 34ch; line-height: 1.55; }
  .footer-col .t { font: 700 11px/1 var(--font-mono); letter-spacing: 1.2px; text-transform: uppercase; color: var(--ink-faint); margin-bottom: 16px; }
  .footer-col a { display: block; text-decoration: none; color: var(--ink-dim); font-size: 13.5px; margin-bottom: 11px; transition: color .15s ease; }
  .footer-col a:hover { color: var(--violet); }
  .footer-bottom { max-width: 1120px; margin: 0 auto; padding: 22px 24px 0; display: flex; justify-content: space-between; align-items: center; gap: 12px; flex-wrap: wrap; border-top: 1px solid var(--line); font-size: 12.5px; color: var(--ink-faint); }
`;

const NAV = `<header class="nav" id="siteNav">
    <div class="nav-inner">
      <a class="brand" href="./index.html">
        <span class="mark">${LOGO_USE}</span>
        <span class="wordmark">FRAGIFY</span>
      </a>
      <nav class="nav-links">
        <a href="./index.html#tournaments">Tournaments</a>
        <a href="./index.html#fragcoins">FragCoins</a>
        <a href="./index.html#leaderboard">Leaderboard</a>
        <a href="./index.html#host">Host</a>
        <a href="./index.html#early-access">Contact</a>
      </nav>
      <a class="cta-btn" href="mailto:${SUPPORT_EMAIL}?subject=Early%20Access%20Request">Request Early Access</a>
...
const FOOTER = `<footer>
    <div class="footer-grid">
      <div>
        ...
        <p class="footer-tag">Free-to-enter esports tournaments for Free Fire, BGMI, COD Mobile and Valorant.</p>
      </div>
      <div class="footer-col">
        ...
        <a href="./index.html#fragcoins">FragCoins</a>
        <a href="./index.html#leaderboard">Leaderboard</a>
        <a href="./index.html#host">Host a tournament</a>
      </div>
      <div class="footer-col">
    ...
    <div class="footer-bottom">
      ...
      <span>Made for the Indian mobile esports scene.</span>
    </div>
  </footer>`;
...
function page({ title, heading, intro, bodyHtml }) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<body>
  ...
  <div class="page">
    <div class="wrap">
      <h1 class="reveal">${esc(heading)}</h1>
      <p class="updated reveal">Last updated: ${LAST_UPDATED}</p>
      <p class="intro reveal">${esc(intro)}</p>

      ${bodyHtml}

      <div class="doc-card reveal">
        <h2><span class="ico">✉️</span>Contact Us</h2>
        <p>Questions? Reach us at <a class="inline" href="mailto:${SUPPORT_EMAIL}">${SUPPORT_EMAIL}</a>
...
console.log('Built legal pages -> marketing-site/');
console.log('  privacy-policy.html  terms.html  delete-account.html');
console.log('(index.html is hand-authored -- this script does not touch it)');
